...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                  soft constraints                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	cost_type(COST_TYPE,CID) :- assign(CID,RID), case(CID, CASETYPE, EFFORT, DAMAGE, PCODE, CASEPAYMENT), referee(RID, RTYPE, MAX_WL, PREV_WL, REF_PREV_PAYM), prefType(RID, PREF_CASETYPE, PREF)
	                               , PREF >= 1, COST_TYPE = 3-PREF.
	
	cost_region(COST_REGION,CID) :-  assign(CID,RID), case(CID, CASETYPE, EFFORT, DAMAGE, PCODE, CASEPAYMENT), referee(RID, RTYPE, MAX_WL, PREV_WL, REF_PREV_PAYM), prefRegion(RID, PREF_PCODE, PREF)
	...
	wl_divergence(WL_DIVERGENCE,CID) :- assign(CID,RID), case(CID, CASETYPE, EFFORT, DAMAGE, PCODE, CASEPAYMENT), referee(RID, RTYPE, MAX_WL, PREV_WL, REF_PREV_PAYM)
  	...
  	%:- assign(CID,RID), avg_ext_ref_total_payment(AVG_TOTAL_EXT_PAYM), ext_ref_total_payment(RID, REF_TOTAL_PAYM), REF_TOTAL_PAYM>AVG_TOTAL_EXT_PAYM.
 	...
 	#minimize{((COST_TYPE*34)+(COST_REGION*34)+(CASEPAYMENT*16)-(WL_DIVERGENCE*9)-(P_DIVERGENCE*7))@1,CID : 
 							cost_type(COST_TYPE,CID), cost_region(COST_REGION,CID), internal_over_ext(CASEPAYMENT,CID),
 							wl_divergence(WL_DIVERGENCE,CID), payment_divergence(P_DIVERGENCE,CID), assign(CID,RID), 
